...
export default function UserList() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await axios.get("https://be.sparkx1.pro/api/getalluser");
  ...
  const handleSuspendToggle = async (id) => {
    try {
      const response = await axios.put(
        `https://be.sparkx1.pro/api/admin/users/${id}/toggle-status`
      );
  ...
  const handleDeleteUser = async (id) => {
    ...
    try {
      const response = await axios.delete("https://be.sparkx1.pro/api/delete", {
        data: { userId: id },
      });
