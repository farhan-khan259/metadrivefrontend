...
export default function UserDetails() {
        ...
        try {
          ...
          if (userId && userId !== "userdetails") {
            const response = await axios.get(
              `https://be.sparkx1.pro/api/admin/users/${userId}`
            );
  ...
  useEffect(() => {
    const fetchTeamData = async () => {
      ...
      try {
        setTeamLoading(true);
        const res = await axios.post("https://be.sparkx1.pro/team", {
          userId: user._id,
        });
    ...
    if (!isNaN(amount) && amount > 0) {
      try {
        const response = await axios.post(
          `https://be.sparkx1.pro/api/addBalanceByAdmin`,
          {
            userId: user._id,
            balance: amount,
          }
  ...
  const handleSubtractBalance = async () => {
    ...
    if (!isNaN(amount) && amount > 0) {
      ...
      try {
        const response = await axios.post(
          `https://be.sparkx1.pro/api/addSubtractByAdmin`,
          {
            userId: user._id,
            balance: amount,
          }
  ...
  const handleDeletePlan = async () => {
    if (window.confirm("Are you sure you want to delete this user's plan?")) {
      try {
        const response = await axios.post(
          `https://be.sparkx1.pro/api/admindeleteplainuser`,
          { userId: user._id }
        );
  ...
  const totalTeamCommission = teamData
    ? Math.floor(teamData.commissionSummary?.grandTotalCommission || 0)
    : 0;
